...
func TestFindMatchingContentForV2Annotation(t *testing.T) {
	...
	contentList, found, err := contentByConceptDriver.GetContentForConcept(MSJConceptUUID, RequestParams{defaultLimit, 0, 0, false})
...
func TestFindMatchingContentForV1Annotation(t *testing.T) {
	...
	contentByConceptDriver := NewContentByConceptService(db)
	contentList, found, err := contentByConceptDriver.GetContentForConcept(MetalMickeyConceptUUID, RequestParams{defaultLimit, 0, 0, false})
...
func TestFindMatchingContentForV2AnnotationWithLimit(t *testing.T) {
	...
	contentByConceptDriver := NewContentByConceptService(db)
	contentList, found, err := contentByConceptDriver.GetContentForConcept(MSJConceptUUID, RequestParams{1, 0, 0, false})
	assert.NoError(err, "Unexpected error for concept %s", MSJConceptUUID)
	assert.True(found, "Found no matching content for concept %s", MSJConceptUUID)
	assert.Equal(1, len(contentList), "Didn't get the same list of content")
	assertListContainsAll(assert, contentList, getExpectedContent())
...
func TestRetrieveNoContentForV1AnnotationForExclusiveDatePeriod(t *testing.T) {
	...
	contentList, found, err := contentByConceptDriver.GetContentForConcept(MetalMickeyConceptUUID, RequestParams{defaultLimit, fromDate.Unix(), toDate.Unix(), false})
	assert.NoError(err, "Unexpected error for concept %s", MetalMickeyConceptUUID)
...
func TestRetrieveNoContentWhenThereAreNoContentForThatConcept(t *testing.T) {
	...
	contentByConceptDriver := NewContentByConceptService(db)
	content, found, err := contentByConceptDriver.GetContentForConcept(MSJConceptUUID, RequestParams{defaultLimit, 0, 0, false})
	assert.NoError(err, "Unexpected error for concept %s", MSJConceptUUID)
...
func TestBrandsDontReturnParentContent(t *testing.T) {
	...
	contentByConceptDriver := NewContentByConceptService(db)
	contentList, found, err := contentByConceptDriver.GetContentForConcept(OnyxPikeBrandUUID, RequestParams{defaultLimit, 0, 0, false})
...
func TestContentIsReturnedFromAllLeafNodesOfConcordance(t *testing.T) {
	...
	for _, uuid := range idsToCheck {
		contentList, found, err := contentByConceptDriver.GetContentForConcept(uuid, RequestParams{defaultLimit, 0, 0, false})
...
func TestContentIsReturnedFromAllLeafNodesOfConcordanceWithDateRestrictions(t *testing.T) {
	...
	for _, uuid := range idsToCheck {
		contentList, found, err := contentByConceptDriver.GetContentForConcept(uuid, RequestParams{defaultLimit, 1372550400, 1388448000, false})
